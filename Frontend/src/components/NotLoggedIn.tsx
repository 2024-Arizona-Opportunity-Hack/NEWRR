...
const NotLoggedIn: React.FC<NotLoggedInProps> = ({ hasUser }) => {
    ...
    const title = hasUser ? "You do not have access to this page" : "You are not logged in";
    ...
    return (
        <div className="flex justify-center items-center min-h-screen">
            <div className='bg-darkestgreen px-4 py-8 w-96 rounded-lg flex flex-col items-between shadow-2xl'>
                <p className="text-4xl font-bold text-white mb-4 text-center">{title}</p>
                <p className='text-white text-center mb-4'>Please login to continue.</p>
                <button onClick={handleLogout} className="bg-white text-darkestgreen px-4 py-2 rounded-md">Click here to log in</button>
            </div>
      </div>
    );
  };
